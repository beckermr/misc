...
def _process_file(filename, ebvlim, force_remake=False):
    nside = 128
    nbins = 40
    bins = np.linspace(0, 3653, nbins+1)

    runname = os.path.basename(filename).replace('.db', '')
    oname = runname + "_ebvlim%d_nside%d_bins%d.fits" % (int(ebvlim * 1000), nside, nbins)

    if (not os.path.exists(oname)) or force_remake:
        print("starting %s" % os.path.basename(filename), flush=True)

        metric = RIZDetectionCoaddExposureTime(bins=bins, ebvlim=ebvlim)
...
def main():
    ...
    jobs = []
    for ebvlim in [0.1, 0.125, 0.15, 0.175, 0.2]:
        jobs = jobs + [
            joblib.delayed(_process_file)(fname, ebvlim, force_remake=force_remake)
            for fname in fnames
        ]
