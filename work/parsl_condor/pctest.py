...
while n_left > 0:
    for i in range(len(mapped_results)):
        ...
        if f.done():
            ...
            print("[%06ds] eta|done|left:" % (int(time.time() - START)), eta, n_tot - n_left, n_left, flush=True)
...
print("total:", sum(res), sum(2*x for x in range(0, int(sys.argv[1]))), flush=True)

print("done w/ loop:", time.time() - start, flush=True)
